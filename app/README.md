...
`npm run build` 가 아래 **레포 자체 제작(학습용 재구성) + 합성 생성** 콘텐츠를
파싱·생성해 `questions.json` 을 만듭니다. 외부 기출 원문을 가져오지 않습니다.
- `모의고사/` → 모의고사 문항(정답·해설 포함)
- `문제집/유형별/*.md` → 유형별 문항
- `app/data/original-questions.json` → 손으로 작성한 오리지널 문항
- `app/generate-questions.mjs` → **과목당 250문항(총 750)** 을 결정론적으로 생성
  (`app/data/generated-questions.json`). 숫자 계산·형식논리·합성 지문이라
  정답이 구성상 보장되며, 시드가 고정돼 빌드마다 동일하게 재현된다.

현재 번들: 총 **846문항** (언어논리 287 · 자료해석 281 · 상황판단 278).
